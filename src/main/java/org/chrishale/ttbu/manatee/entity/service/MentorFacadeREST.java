...
import org.chrishale.ttbu.manatee.entity.Mentor;
 ...
 */
@Stateless
@Path("org.chrishale.ttbu.manatee.entity.mentor")
public class MentorFacadeREST extends AbstractFacade<Mentor> {

    @PersistenceContext(unitName = "org.chrishale.ttbu_manatee_war_1.0-SNAPSHOTPU")
    private EntityManager em;

    public MentorFacadeREST() {
        super(Mentor.class);
        System.out.println("MentorFacadeREST object created.");
    ...
    @Path("{id}")
    @Consumes({MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON})
    public void edit(@PathParam("id") String id, Mentor entity) {
        super.edit(entity);
    }
    ...
    public Mentor find(@PathParam("id") String id) {
    ...
    @Path("{from}/{to}")
    @Produces({MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON})
    public List<Mentor> findRange(@PathParam("from") Integer from, @PathParam("to") Integer to) {
